...
require_once __DIR__ . '/horas_extra.php';
...
function ht_carregar_funcionarios($conn, $funcionarioId = null)
{
    ...
    $sql = "SELECT f.id, f.nome, f.numero_mecanografico, f.email, f.telefone, f.funcao, f.carga_horaria_semanal, $selectEquipa
            FROM funcionarios f
            $joinEquipa
            WHERE f.estado NOT IN ('inativo', 'arquivado')";
    ...
    foreach ($registosPorDia as $fid => $dias) {
        foreach ($dias as $data => $movimentos) {
            ...
            if ($minutos > 0) {
                ...
                $funcionarios[$fid]['minutos_extra'] += he_calcular_extra_minutos(
                    $minutos,
                    he_carga_diaria_minutos($funcionarios[$fid])
                );
